...
public class OAuthService {
    private final Map<OAuthProvider, kr.co.pinup.oauth.OAuthApiClient> clients;
    private final AppLogger appLogger;

    public OAuthService(List<kr.co.pinup.oauth.OAuthApiClient> clients, AppLogger appLogger) {
        this.clients = clients.stream().collect(
                Collectors.toUnmodifiableMap(kr.co.pinup.oauth.OAuthApiClient::oAuthProvider, Function.identity())
        );
        this.appLogger = appLogger;
    }

    public Pair<kr.co.pinup.oauth.OAuthResponse, kr.co.pinup.oauth.OAuthToken> request(OAuthLoginParams params) {
        ...
        kr.co.pinup.oauth.OAuthApiClient client = Optional.ofNullable(clients.get(params.oAuthProvider()))
                .orElseThrow(() -> {
        ...
        kr.co.pinup.oauth.OAuthToken token = Optional.ofNullable(client.requestAccessToken(params))
                .orElseThrow(() -> {
    ...
    public OAuthResponse isAccessTokenExpired(OAuthProvider oAuthProvider, String accessToken) {
        kr.co.pinup.oauth.OAuthApiClient client = Optional.ofNullable(clients.get(oAuthProvider))
                .orElseThrow(() -> {
    ...
    public kr.co.pinup.oauth.OAuthToken refresh(OAuthProvider oAuthProvider, String refreshToken) {
        kr.co.pinup.oauth.OAuthApiClient client = Optional.ofNullable(clients.get(oAuthProvider))
                .orElseThrow(() -> {
                    String msg = oAuthProvider + "는 지원하지 않는 OAuth 제공자입니다.";
                    appLogger.warn(new WarnLog(msg));
